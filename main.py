...
class MenuBar(tk.Frame):
    ...
    def __init__(self, master):
        ...
        if not os.path.exists(UserConfiguration.user_settings_file_path):
            shutil.copy(UserConfiguration.default_user_settings_path,
                        UserConfiguration.user_settings_file_path)
        ...
        self.monitoring_job_dir = os.path.join(
            survey_config.root_job_directory, survey_config.current_year, survey_config.default_survey_type)
        ...
        self.file_sub_menu.add_command(
            label="Open GSI...", command=self.choose_gsi_file)
        self.file_sub_menu.add_command(
            label="Print GSI", command=self.print_gsi)
        self.file_sub_menu.add_command(
            label="Create Dated Directory...", command=lambda: self.new_dated_directory(False))
        self.file_sub_menu.add_command(
            label="Choose Dated Directory...", command=self.choose_dated_directory)
        self.file_sub_menu.add_command(
            label="Choose Compnet Job Directory...", command=self.choose_compnet_directory)
        self.file_sub_menu.add_command(
            label="Create Job Directory...", command=self.new_job_directoy)
        self.file_sub_menu.add_command(
            label="Import SD Data", command=self.import_sd_data)
        self.file_sub_menu.add_separator()
        self.file_sub_menu.add_command(
            label="Monitoring - Create (beta)", command=self.monitoring_create)
        self.file_sub_menu.add_command(
            label="Monitoring - Update Coordinates", command=self.monitoring_update_coords, state="disabled")
        self.file_sub_menu.add_command(
            label="Monitoring - Update Labels", command=self.monitoring_update_labels, state="disabled")
        ...
        self.file_sub_menu.add_command(
            label="Settings", command=self.configure_survey)

        self.menu_bar.add_cascade(label="File", menu=self.file_sub_menu)

        # Edit menu
        self.edit_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.edit_sub_menu.add_command(
            label="Delete all 2D Orientation Shots", command=self.delete_orientation_shots)
        self.edit_sub_menu.add_command(
            label="Change point name...", command=self.change_point_name)
        self.edit_sub_menu.add_command(
            label="Change target height...", command=self.change_target_height)
        self.edit_sub_menu.add_command(
            label="(BETA) Change station height...", command=self.change_station_height)
        self.edit_sub_menu.add_separator()
        self.edit_sub_menu.add_command(
            label="Prism Constant - Fix single...", command=self.prism_constant_update_manually)
        self.edit_sub_menu.add_command(
            label="Prism Constant - Fix batch ...", command=self.prism_constant_update_batch)

        self.menu_bar.add_cascade(
            label="Edit Survey", menu=self.edit_sub_menu, state="disabled")

        # Check menu
        self.check_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.check_sub_menu.add_command(
            label="Check Control Naming ", command=self.check_control_naming)
        self.check_sub_menu.add_command(
            label="Check Target Naming ", command=self.check_target_naming)
        self.check_sub_menu.add_command(
            label="Check Prism Constants", command=self.check_prism_constants)
        self.check_sub_menu.add_command(
            label="Check Target Heights", command=self.check_target_heights)
        self.check_sub_menu.add_command(
            label="Check FL-FR", command=self.check_FLFR)
        self.check_sub_menu.add_command(
            label="Check Tolerances (3D)", command=self.check_3d_survey)
        self.check_sub_menu.add_command(
            label="Check All", command=self.check_3d_all)
        self.check_sub_menu.add_separator()
        self.check_sub_menu.add_command(
            label="Check Double Doubles", command=self.check_2d_doubles)
        self.check_sub_menu.add_separator()
        self.check_sub_menu.add_command(
            label="Compare with a similar survey...", command=self.compare_survey)
        self.check_sub_menu.add_separator()
        self.check_sub_menu.add_command(
            label="Query GSI...", command=self.display_query_input_box)
        self.menu_bar.add_cascade(
            label="Check Survey", menu=self.check_sub_menu, state="disabled")

        # Compnet menu
        self.compnet_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.compnet_sub_menu.add_command(
            label="Setup New Compnet Job ...", command=self.create_compnet_job_folder)
        self.compnet_sub_menu.add_command(
            label="Update Fixed File...", command=self.update_fixed_file)
        self.compnet_sub_menu.add_command(
            label="Weight STD File ...", command=self.weight_STD_file)
        self.compnet_sub_menu.add_separator()
        self.compnet_sub_menu.add_command(
            label="Compare CRD Files...", command=self.compare_crd_files)
        self.compnet_sub_menu.add_command(
            label="Create control only GSI", command=self.create_control_only_gsi)
        self.compnet_sub_menu.add_command(
            label="Combine/Re-order GSI Files", command=self.combine_gsi_files)

        self.menu_bar.add_cascade(label="Compnet", menu=self.compnet_sub_menu)

        # Utilities menu
        self.utility_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.utility_sub_menu.add_command(
            label="Export CSV", command=self.export_csv)
        self.utility_sub_menu.add_command(
            label="Create popup CSV from .ASC file", command=self.create_CSV_from_ASC)
        self.utility_sub_menu.add_command(
            label="Create popup CSV from .CRD file", command=self.create_CSV_from_CRD)
        self.utility_sub_menu.add_command(
            label="Copy todays GPS to GNSS temp directory", command=self.copy_gps_to_gnss_temp)
        self.utility_sub_menu.add_command(
            label="Create printable list of change points", command=self.create_list_of_change_points)
        self.utility_sub_menu.add_command(
            label="Combine/Re-order GSI Files", command=self.combine_gsi_files)

        self.menu_bar.add_cascade(
            label="Utilities", menu=self.utility_sub_menu)

        # Job Tracker
        self.job_tracker_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.job_tracker_sub_menu.add_command(
            label="Track/Create a Job", command=self.job_tracker_open)
        self.job_tracker_sub_menu.add_command(
            label="Open in excel", command=self.job_tracker_open_excel)
        self.job_tracker_sub_menu.add_command(
            label="Hide", command=self.job_tracker_hide)
        self.menu_bar.add_cascade(
            label="Job Tracker", menu=self.job_tracker_sub_menu)

        # Help menu
        self.help_sub_menu = tk.Menu(self.menu_bar, tearoff=0)
        self.help_sub_menu.add_command(
            label="Manual", command=self.open_manual)
        self.help_sub_menu.add_command(
            label="About", command=self.display_about_dialog_box)
    ...
    def choose_gsi_file(self):
        ...
        try:
            ...
            if self.compnet_working_dir != "":  # compnet job has been setup
                ...
                intial_directory = os.path.join(
                    survey_config.todays_dated_directory, "TS")

            MenuBar.filename_path = tk.filedialog.askopenfilename(
                initialdir=intial_directory, title="Select file", filetypes=[("GSI Files", ".gsi")])
            ...
            tk.messagebox.showerror(
                "Survey Assist", "An unexpected error has occurred\n\nProblem opening up the GSI file\n\n" + str(ex))
            return
        ...
        try:
            ...
            if folder_selected is None:
                folder_selected = filedialog.askdirectory(
                    parent=self.master, initialdir=default_path, title='Please select the job directory')
            ...
            if dated_directory_path:
                ...
                if (os.path.isdir(dated_directory_path + '/TS')) & (os.path.isdir(dated_directory_path + '/GPS')) & (
                    ...
                    tk.messagebox.showwarning(
                        "Survey Assist", "The directory you have selected doesn't appear to be a valid dated directory.")
                    return
        except Exception as ex:
            print("Unexpected error has occurred\n\n" + str(ex))
            logger.exception(
    ...
    def choose_compnet_directory(self):
        ...
        compnet_monitoring_dir = os.path.join(
            survey_config.compnet_data_dir, survey_config.current_year, survey_config.default_survey_type)
    ...
    def new_job_directoy(self):
        ...
        initial_dir = os.path.join(survey_config.root_job_directory,
                                   survey_config.current_year, survey_config.default_survey_type)
    ...
    def import_sd_data(self):
        ...
        if not SDCard.user_SD_dir_exists(user_sd_directory):
            ...
            if not SDCard.user_SD_dir_exists(usb_root_directory):
                ...
                tk.messagebox.showinfo(
                    "IMPORT SD DATA", "Can't find your SD card drive.\n\nPlease select your SD drive.")

                user_sd_directory = tkinter.filedialog.askdirectory(
                    parent=self.master, initialdir='C:\\', title='Please choose your SD card drive')

                if user_sd_directory:
                    self.user_config.update(
                        UserConfiguration.section_file_directories, 'user_sd_root', user_sd_directory)
        ...
        if not todays_dated_directory:
            ...
            if all(x in folders for x in ['GPS', 'TS', 'OUTPUT', 'OTHER']):
                ...
                tk.messagebox.showerror(
                    "COPYING SD DATA", "The root job folder is not a dated dirctory.  Please try re-importing")
                return
        ...
        filename_paths = set(
            [file.filepath for file in sd_card.get_list_all_todays_files()])
        filenames_txt_list = ""
        confirm_msg = "The following " + \
            str(len(filename_paths)) + " files will be copied over to " + \
        ...
        if tk.messagebox.askokcancel(message=confirm_msg):
            ...
            try:
                ...
                if sd_card.get_todays_gps_files():
                    for file in sd_card.get_todays_gps_files():
                        import_path = os.path.join(
                            import_root_directory, 'GPS', file.basename)
                    ...
                    for file in sd_card.get_todays_ts_15_files():
                        import_path = os.path.join(
                            import_root_directory, 'TS', 'TS15', file.basename)
                        if os.path.isdir(file.filepath):
                            shutil.copytree(file.filepath, import_path)
                        else:
                            shutil.copy(file.filepath, import_path)
                    ...
                    for file in sd_card.get_todays_ts_60_files():
                        import_path = os.path.join(
                            import_root_directory, 'TS', 'TS60', file.basename)
                        if os.path.isdir(file.filepath):
                            shutil.copytree(file.filepath, import_path)
                        else:
                            shutil.copy(file.filepath, import_path)
                    ...
                    for file in sd_card.get_todays_ms_60_files():
                        import_path = os.path.join(
                            import_root_directory, 'TS', 'MS60', file.basename)
                        if os.path.isdir(file.filepath):
                            shutil.copytree(file.filepath, import_path)
                        else:
                            shutil.copy(file.filepath, import_path)
                    ...
                    for file in rail_monitoring_files:
                        ...
                        import_path = os.path.join(
                            import_root_directory, 'TS', ts_used, file.basename)
                        if os.path.isdir(file.filepath):
                            shutil.copytree(file.filepath, import_path)
                        else:
                            shutil.copy(file.filepath, import_path)
                    ...
                    tk.messagebox.showerror(
                        "COPYING SD DATA", "Unexpected error has occurred: .\n\nPlease notify the developer.")
                ...
                logger.exception(
                    "Importing SD Data\n\nException\n\n" + str(ex))
                tk.messagebox.showerror(
                    "COPYING SD DATA", "Problem copying files across.  Please copy files over manually.\n\n" + str(ex))
                ...
                tk.messagebox.showinfo(
                    "COPYING SD DATA", "SD Card transfer is complete.  It is now safe to remove the SD card.")
            finally:
    ...
    def print_gsi(self):
        ...
        if not os.path.exists(users_print_gsi_excel_filepath):
            shutil.copy(template_print_gsi_excel_filepath,
                        users_print_gsi_excel_filepath)

        # highlight colors
        orange_highlight = PatternFill(
            start_color='FFD966', end_color='FFD966', fill_type='solid')
        green_highlight = PatternFill(
            start_color='C6E0B4', end_color='C6E0B4', fill_type='solid')
        blue_highlight = PatternFill(
            start_color='DDEBF7', end_color='DDEBF7', fill_type='solid')

        if not MenuBar.filename_path:
            tk.messagebox.showinfo(
                "Print GSI", "Please open up a GSI file first.")
            return

        try:
            workbook = load_workbook(
                users_print_gsi_excel_filepath, read_only=False)
            ...
            for excel_line_number, formatted_line in enumerate(gsi.formatted_lines, start=4):
                ...
                if gsi.is_station_setup(formatted_line):
                    ...
                    excel_sheet['A' + str(excel_line_number)
                                ] = formatted_line['Point_ID']
                    excel_sheet['E' + str(excel_line_number)
                                ] = formatted_line['Prism_Constant']
                    excel_sheet['F' + str(excel_line_number)
                                ] = formatted_line['STN_Easting']
                    excel_sheet['G' + str(excel_line_number)
                                ] = formatted_line['STN_Northing']
                    excel_sheet['H' + str(excel_line_number)
                                ] = formatted_line['STN_Elevation']
                    excel_sheet['J' + str(excel_line_number)
                                ] = formatted_line['STN_Height']

                    # Highlight and style row
                    for col in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']:
                        excel_sheet[col + str(excel_line_number)
                    ...
                    excel_sheet['B' + str(excel_line_number)
                                ] = formatted_line['Horizontal_Angle']
                    excel_sheet['C' + str(excel_line_number)
                                ] = formatted_line['Vertical_Angle']
                    excel_sheet['E' + str(excel_line_number)
                                ] = formatted_line['Prism_Constant']
                    excel_sheet['I' + str(excel_line_number)
                                ] = formatted_line['Target_Height']
                    ...
                    excel_sheet['D' + str(excel_line_number)
                                ] = formatted_line['Slope_Distance']
                    excel_sheet['E' + str(excel_line_number)
                                ] = formatted_line['Prism_Constant']
                    excel_sheet['F' + str(excel_line_number)
                                ] = formatted_line['Easting']
                    excel_sheet['G' + str(excel_line_number)
                                ] = formatted_line['Northing']
                    excel_sheet['H' + str(excel_line_number)
            ...
            logger.exception(
                'Print GSI excel spreadsheet not found\n\n' + str(ex))
            ...
            logger.exception(
                'Print GSI excel spreadsheet is currently open\n\n' + str(ex))
            ...
            logger.exception(
                'Unknown error occurred in Print GSI\n\n' + str(ex))
            tk.messagebox.showerror(
                "ERROR", "Unknown error occurred printing the GSI.  Please contact the developer")
    ...
    def copy_over_gsi_to_edited_directory(self, gsi_file, import_path, is_rail_survey):
        ...
        if is_rail_survey:
            edited_filename_path = ts_root_dir + '/EDITING/' + gsi_file.basename_no_ext + \
                '_' + self.ts_used + '_' + Today.todays_date_reversed + '_EDITED.GSI'
        else:
            edited_filename_path = ts_root_dir + '/EDITING/' + \
                gsi_file.basename_no_ext + '_EDITED.GSI'
    ...
    def monitoring_create(self):
        ...
        try:
            ...
            station_list_dict = gsi.get_list_of_station_setups(
                gsi.formatted_lines)
            ...
            for line_number, station_name in station_list_dict.items():
                ...
                station_shots_dict = gsi.get_all_shots_from_a_station_including_setup(
            ...
            for station_name, coordinate_list in csv_dict.items():
                monitoring_files_dir = os.path.join(
                    os.getcwd(), 'Monitoring Files')
                out_csv = os.path.join(
                    monitoring_files_dir, station_name + '.csv')
            ...
            tk.messagebox.showinfo(
                "Create Monitoring Files", "Monitoring files have been created at " + monitoring_files_dir)
    ...
    def check_3d_survey(self):
        ...
        try:
            errors, error_points = gsi.check_3D_survey(
                database.conn, survey_config)
            ...
            logger.exception(
                'Error creating executing SQL query\n\n' + str(ex))
            tk.messagebox.showerror(
                "Error", 'Error checking survey tolerance:\n\n' + str(ex))
    ...
    def check_control_naming(self):
        ...
        try:
            ...
            tk.messagebox.showerror(
                "Error", 'Error checking control naming:\n\n' + str(ex))

    def check_target_naming(self):

        try:
            error_text, error_line_numbers = gsi.check_target_naming()

            # display error dialog box
            tkinter.messagebox.showinfo(
                "Checking Target Naming for each setup", error_text)
            gui_app.list_box.populate(gsi.formatted_lines, error_line_numbers)

        except Exception as ex:
            logger.exception('Error checking target naming\n\n' + str(ex))
            tk.messagebox.showerror(
                "Error", 'Error checking target naming:\n\n' + str(ex))
    ...
    def check_prism_constants(self):
        ...
        try:
            ...
            tk.messagebox.showerror(
                "Error", 'Error checking prism constants:\n\n' + str(ex))
    ...
    def check_target_heights(self):
        ...
        try:
            ...
            tk.messagebox.showerror(
                "Error", 'Error checking target heights:\n\n' + str(ex))
        ...
        try:
            ...
            for gsi_line_number, line in enumerate(gsi.formatted_lines, start=0):
                ...
                if GSI.is_station_setup(line):
                    ...
                    obs_from_station_dict = gsi.get_all_shots_from_a_station_including_setup(
                        station_name, gsi_line_number)
                    points_no_2nd_face, analysed_lines = self.anaylseFLFR(
                        copy.deepcopy(obs_from_station_dict))
            ...
            if display == 'NO':  # don't display results to user - just a popup dialog to let them know there is an issue
                ...
                gui_app.list_box.populate(
                    formatted_gsi_lines_analysis, error_line_number_list)
    ...
    def anaylseFLFR(self, obs_from_station_dict):
        ...
        sorted_obs_from_station_list = sorted(
            obs_from_station_list, key=lambda i: i['Point_ID'])
                ...
                if obs_line_1_dict['Point_ID'] == obs_line_2_dict['Point_ID']:
                    ...
                    for key, obs_line_1_field_value_str in obs_line_1_dict.items():
                        ...
                        if key == 'Timestamp':
                            ...
                            obs_line_1_field_value = get_numerical_value_from_string(
                                obs_line_1_field_value_str, field_type, precision)

                            obs_line_2_field_value = get_numerical_value_from_string(
                                obs_line_2_field_value_str, field_type, precision)
                            if key == 'Horizontal_Angle':
                                angular_diff = decimalize_value(angular_difference(
                                    obs_line_1_field_value, obs_line_2_field_value, 180), '3dp')
                            else:  # key is vertical angle:
                                obs_angular_diff = angular_difference(
                                    obs_line_2_field_value, obs_line_1_field_value, 0)
                                angular_diff = decimalize_value(
                                    angular_difference(obs_angular_diff, -360.00, 0), '3dp')

                            angle_dms = angle_decimal2DMS(angular_diff)
                            # make 3dp precision for 4dp shots so it formats correctly
                            obs_line_2_dict[key] = GSI.format_angles(
                                angle_dms, '3dp')

                        elif key == 'Prism_Constant':
                            obs_line_2_dict[key] = str(
                                int(obs_line_1_dict[key]) - int(obs_line_1_dict[key]))
                        elif key == 'Point_ID':
                            pass
                            ...
                            if (obs_line_1_field_value != "") and (obs_line_2_field_value != ""):
                                float_diff_str = str(decimalize_value(
                                    obs_line_1_field_value - obs_line_2_field_value, precision))
                                float_diff_str = self.check_diff_exceed_tolerance(
                                    key, float_diff_str)
    ...
    def check_3d_all(self):
        ...
        if not MenuBar.filename_path:
            tk.messagebox.showinfo(
                "Check Survey", "Please open up a GSI file first.")
            return
    ...
    def change_target_height(self):
        ...
        if not selected_items:
            # no lines selected
            tkinter.messagebox.showinfo(
                "Updating Target Height", "Please select at least one line to update")
            return

        for selected_item in selected_items:
            line_number = gui_app.list_box.list_box_view.item(selected_item)[
                'values'][0]

            # Check to make sure that the selected line is not a STN setup
            if gsi.is_station_setup(gsi.get_formatted_line(line_number)):
                tk.messagebox.showinfo(
                    "Updating Target Height", "Please select lines that contain only target heights, NOT a station height")
                return
            elif gsi.is_orientation_shot(gsi.get_formatted_line(line_number)):
                tk.messagebox.showinfo(
                    "Updating Target Height", "Please select lines that contain only target heights, NOT an orientation line")
                return
    ...
    def change_station_height(self):
        ...
        for selected_item in selected_items:
            ...
            if not gsi.is_station_setup(gsi.get_formatted_line(line_number)):
                tk.messagebox.showinfo(
                    "Updating Station Height", "Please select a line that contains only station setup")
                return

        if len(selected_items) > 1:
            tkinter.messagebox.showinfo(
                "Updating Station Height", "Please select only ONE station height to update at a time")
            return
        elif not selected_items:
            # no lines selected
            tkinter.messagebox.showinfo(
                "Updating Station Height", "Please select at least one line to update")
            return
    ...
    def prism_constant_update_manually(self):
        ...
        try:
            ...
            if selected_items:
                for selected_item in selected_items:
                    line_numbers_to_ammend.append(
                        gui_app.list_box.list_box_view.item(selected_item)['values'][0])
            else:
                # no lines selected
                tkinter.messagebox.showinfo(
                    "Updating Prism Constant", "Please select at least one line to update")
                return
    ...
    def check_2d_doubles(self):
        ...
        try:
            if not MenuBar.filename_path:
                tk.messagebox.showinfo(
                    "Survey Assist", "Please open up a GSI file first.")
                return
            ...
            station_points_dict = gsi.get_list_of_station_setups(
                gsi.formatted_lines)
            for gsi_stn_line_number, station_name in station_points_dict.items():
                ...
                station_shots_dict = gsi.get_all_shots_from_a_station_including_setup(
                    station_name, gsi_stn_line_number)
                ...
                if len(doubles_list) == 0:
                    ...
                    error_text += station_name + \
                        ":  Only one double double was found for this setup:\n              " + \
                        doubles_list[0] + "\n"
                elif len(doubles_list) > 2:
                    qty = str(len(doubles_list))
                    error_text += station_name + ":  " + qty + \
                        " double doubles were found for this setup:\n"
    ...
    def compare_survey(self):
        try:
            if not MenuBar.filename_path:
                tk.messagebox.showinfo(
                    "Compare Survey", "Please open up a GSI file first.")
                return
                ...
                for line_number, current_gsi_line in enumerate(gsi.formatted_lines, start=1):
                    ...
                    if old_compare_values_list['old_point_id'] == current_point_id:
                        ...
                        if old_compare_values_list['Prism_Constant'] != current_PC:
                            points_pc_diff_dict[current_point_id] = {
                                'current_pc': current_PC, 'old_pc': old_compare_values_list['Prism_Constant']}
                        ...
                        print(current_point_id + "  " +
                              old_compare_values_list['Target_Height'])
            ...
            if points_pc_diff_dict or points_target_height_diff_dict or diff_points:
                ...
                if diff_points:
                    ...
                    for point in diff_points:
                        ...
                        point_line_number_errors = gsi.get_point_name_line_numbers(
                            point)
            ...
            gui_app.list_box.populate(
                gsi.formatted_lines, list(line_number_errors))
    ...
    def create_compnet_job_folder(self):
        ...
        try:
            ...
            print(ex)
            tkinter.messagebox.showinfo(
                "Creating Compnet Jobs Files", compnet_raw_dir + " directory does not exist")
            return
        ...
        try:
            ...
            tkinter.messagebox.showinfo(
                "Creating Compnet Jobs Files", "Directory " + current_path + " already exists")
            return
        except Exception as ex:
            print(ex)
            tkinter.messagebox.showinfo(
                "Creating Compnet Jobs Files", current_path + " directory does not exist")
            return
        else:
    ...
    def create_CSV_from_ASC(self):
        ...
        try:
            ...
            tk.messagebox.showerror(
                "Survey Assist", "Please close the previous popup CSV and try again. ")
    ...
    def create_CSV_from_CRD(self):
        ...
        try:
            initial_directory = os.path.join(
                survey_config.todays_dated_directory, "OUTPUT")
        ...
        try:
            ...
            if not self.compnet_working_dir:
                ...
                if not self.compnet_working_dir:  # user cancelled
                    tk.messagebox.showinfo(
                        "Copying Compnet Job Files", "A Compnet job directory must be selected before continuing.")
                    return
            ...
            if not survey_config.todays_dated_directory:
                ...
                if not survey_config.todays_dated_directory:  # user cancelled
                    tk.messagebox.showinfo(
                        "Copying Compnet Job Files", "A dated directory must be selected before continuing.")
                    return
            ...
            if tk.messagebox.askokcancel("Copying Compnet Job Files", confirm_msg):
                ...
                tk.messagebox.showinfo(
                    "Copying Compnet Job Files", "Compnet job files successfully copied over.")
            ...
            logger.exception(
                "An unexpected error has occurred\n\ncopy_compnet_job_to_dated_directory()\n\n" + str(ex))
            tk.messagebox.showerror(
                "Copying Compnet Job Files", "AN unexpected error has occured.\n\n" + str(ex))
        ...
        try:
            ...
            if tk.messagebox.askokcancel("Copy GPS files to GNSS Temp", confirm_msg):
                ...
                if not todays_dated_directory:
                    ...
                    todays_GPS_directory = os.path.join(
                        todays_dated_directory, 'GPS')
                ...
                tk.messagebox.showinfo(
                    "Copying GPS files", "GPS files successfully copied over.")
            ...
            logger.exception(
                "An unexpected error has occurred\n\ncopy_gps_to_gnss_temp()\n\n" + str(ex))
            tk.messagebox.showerror(
                "Copying GPS files", "AN unexpected error has occured.\n\n" + str(ex))
    ...
    def create_list_of_change_points(self):
        try:
            ...
            control_points_dict = gsi.get_list_of_station_setups(
                gsi.formatted_lines)
            ...
            for line_number, control_name in control_points_dict.items():
                shots_from_station = gsi.get_all_shots_from_a_station_including_setup(
                    control_name, line_number)
            ...
            try:
                ...
                tk.messagebox.showerror(
                    "Error creating the change point list", str(ex))
    ...
    def job_tracker_open_excel(self):
        ...
        try:
            self.job_tracker_filepath = os.path.join(
                survey_config.root_job_directory, survey_config.current_year, survey_config.job_tracker_filename)
            ...
            logger.exception(
                "An unexpected error has occurred\n\nbtn_job_tracker()\n\n" + str(ex))
            tk.messagebox.showerror(
                "Survey Assist", "Couldn't find the Job Tracker Spreadsheet:\n\n" + self.job_tracker_filepath)
            return
    ...
    def job_tracker_open(self):
        ...
        try:
            gui_app.job_tracker_bar = JobTrackerBar(
                gui_app.main_window, self.user_config.user_initials)
    ...
    def delete_orientation_shots():
        ...
        try:
            ...
            with open(MenuBar.filename_path, "r") as gsi_file:
                ...
                for orientation_line_number in ListBoxFrame.orientation_line_numbers:
                    deleted_lines.append(
                        line_list[int(orientation_line_number) - counter])
            ...
            msg_deleted_lines = str(len(deleted_lines)) + \
                " 2D orientation shots have been deleted"

            # display deleted lines dialog box
            tkinter.messagebox.showinfo(
                "2D Orientation Shots", msg_deleted_lines)
            ...
            # Most likely an corrupted GSI file was selected
            tk.messagebox.showerror(
                "LOADING GSI", 'Error reading GSI File:\n\nThis file is a corrupted or incorrect GSI file')
...
class ConfigDialogWindow:
    ...
    def __init__(self, master):
        ...
        self.entry_northing.insert(
            tkinter.END, survey_config.northing_tolerance)
        self.entry_northing.grid(row=2, column=1, padx=10, pady=5, sticky='w')

        tk.Label(self.dialog_window, text="Height Tolerance: ").grid(
    ...
    def select_sorted_stn_file(self):
        ...
        if self.sorted_stn_file_path != "":
            self.sorted_station_file_btn.config(
                text=os.path.basename(self.sorted_stn_file_path))
    ...
    def save(self):
        ...
        file_directory_dictionary['current_rail_monitoring_file_name'] = self.current_rail_monitoring_file_entry.get(
        ).strip()
        ...
        if not file_directory_dictionary['current_rail_monitoring_file_name']:
            tkinter.messagebox.showinfo(
                "Survey Config", "Please enter a name for the current monitoring file")
        ...
        if input_error:
            ...
            tkinter.messagebox.showinfo(
                "Survey Config", "Please enter a numerical tolerance value")
            ...
            for key, value in precision_dictionary.items():
                survey_config.update(
                    SurveyConfiguration.section_instrument, key, value)

            for key, value in survey_tolerance_dictionary.items():
                survey_config.update(
                    SurveyConfiguration.section_survey_tolerances, key, value)

            for key, value in configuration_dictionary.items():
                survey_config.update(
                    SurveyConfiguration.section_config_files, key, value)

            for key, value in file_directory_dictionary.items():
                survey_config.update(
                    SurveyConfiguration.section_file_directories, key, value)

            survey_config = SurveyConfiguration()

            tkinter.messagebox.showinfo(
                "Survey Config", "Settings updated.\n\nPlease RE-START Survey Assist")
...
class QueryDialogWindow:
    ...
    def __init__(self, master):
        ...
        tk.Label(self.dialog_window, text="Build an SQL 'where' statement:").grid(
            row=0, padx=5, pady=5)
        tk.Label(self.dialog_window, text="Select column:").grid(
            row=1, sticky="W", padx=5, pady=5)
        tk.Label(self.dialog_window, text="Enter a column value:").grid(
        ...
        self.column_entry = ttk.Combobox(
            self.dialog_window, width=18, textvariable=self.column, state='readonly')
        self.column_entry['values'] = gsi.query_column_names
        self.column_entry.bind("<<ComboboxSelected>>",
                               self.column_entry_cb_callback)
        ...
        ok_b = tk.Button(self.dialog_window, text="OK",
    ...
    def column_entry_cb_callback(self, event):
        ...
        self.column_value_entry['values'] = sorted(
            set(gsi.get_column_values(self.column_entry.get())))
    ...
    def ok(self):
        ...
        if column_entry == "":
            tkinter.messagebox.showinfo(
                "GSI Query", "Please enter valid search data")
            logger.info(
        ...
        query_results = self.execute_sql_query(
            database.TABLE_NAME, column_entry, column_value_entry)
        print(query_results)
    ...
    def execute_sql_query(database_table, column_name, column_value):
        ...
        sql_query_text = "SELECT * FROM {} WHERE {}=?".format(
            database_table, column_name)
        ...
        try:
            ...
            logger.exception(
                'Error creating executing SQL query:  {}'.format(sql_query_text))
            tk.messagebox.showerror(
                "Error", 'Error executing this query:\nPlease contact the developer of this program\n\n' + str(ex))
    ...
    def repopulate_list_box(query_results):
        ...
        if query_results is not None:
            ...
            gui_app.list_box.list_box_view.delete(
                *gui_app.list_box.list_box_view.get_children())
            ...
            for query_result in query_results:
                ...
                gui_app.list_box.list_box_view.insert(
                    "", "end", values=query_list)
...
class StatusBar(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.status = tk.Label(
            master, text='Welcome to Survey Assist', relief=tk.SUNKEN, anchor=tk.W)
...
class WorkflowBar(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.btn_diary = tk.Button(
            self.frame, text="Job Diary", command=MenuBar.job_diary)
        ...
        self.btn_import_sd_data = tk.Button(
            self.frame, text="Import SD Data", command=lambda: gui_app.menu_bar.import_sd_data())
        self.btn_import_sd_data.configure(background='#FCF1E1')
        self.btn_open_gsi = tk.Button(
            self.frame, text="Open GSI", command=lambda: gui_app.menu_bar.choose_gsi_file())
        self.btn_open_gsi.configure(background='#FCF1E1')
        self.lbl_edit_gsi = tk.Label(
            self.frame, text="Edit GSI", borderwidth=2, relief="groove", padx=4, pady=4)
        self.lbl_edit_gsi.configure(background='#FCF1E1')
        self.btn_check_survey = tk.Button(
            self.frame, text="Check Survey", command=lambda: gui_app.menu_bar.check_3d_all())
        self.btn_check_survey.configure(background='#FCF1E1')
        self.btn_compare_survey = tk.Button(
            self.frame, text="Compare Survey", command=lambda: gui_app.menu_bar.compare_survey())
        self.btn_compare_survey.configure(background='#FCF1E1')
        self.btn_export_csv = tk.Button(
            self.frame, text="Export GSI", command=lambda: gui_app.menu_bar.export_csv())
        ...
        self.btn_compnet_new_job = tk.Button(
            self.frame, text="Setup New Job", command=lambda: gui_app.menu_bar.create_compnet_job_folder())
        self.btn_compnet_new_job.configure(background='#FCF1E1')
        self.btn_update_fixed_file = tk.Button(
            self.frame, text="Update Fixed File", command=lambda: gui_app.menu_bar.update_fixed_file())
        self.btn_update_fixed_file.configure(background='#FCF1E1')
        self.btn_weight_std_file = tk.Button(
            self.frame, text="Weight STD File", command=lambda: gui_app.menu_bar.weight_STD_file())
        ...
        self.btn_csv_from_crd = tk.Button(
            self.frame, text="Popup CSV from CRD", command=lambda: gui_app.menu_bar.create_CSV_from_CRD())
        self.btn_csv_from_crd.configure(background='#FCF1E1')

        # Redisplay observations button
        self.btn_re_display_gsi = tk.Button(
            self.frame, text="Re-display GSI", command=lambda: gui_app.menu_bar.re_display_gsi())
...
class JobTrackerBar(tk.Frame):
    ...
    def __init__(self, master, user_initials):
        ...
        self.jt_date_btn = tk.Button(
            self.frame, text=self.todays_date, command=self.choose_date)
        ...
        self.jt_calcs_checkbox = tk.Checkbutton(
            self.frame, text='Calcs', variable=self.calcs_checkbox_var, onvalue='1', offvalue='')
        self.jt_calcs_checkbox.configure(background='#d9f2d8')
        self.jt_results_checkbox = tk.Checkbutton(
            self.frame, text='Results', variable=self.results_checkbox_var, onvalue='1', offvalue='')
        self.jt_results_checkbox.configure(background='#d9f2d8')
        self.jt_checked_checkbox = tk.Checkbutton(
            self.frame, text='Checked', variable=self.checked_checkbox_var, onvalue='1', offvalue='')
        self.jt_checked_checkbox.configure(background='#d9f2d8')
        self.jt_sent_checkbox = tk.Checkbutton(
            self.frame, text='Sent', variable=self.sent_checkbox_var, onvalue='1', offvalue='')
        self.jt_sent_checkbox.configure(background='#d9f2d8')
        self.jt_xml_checkbox = tk.Checkbutton(
            self.frame, text='XML', variable=self.xml_checkbox_var, onvalue='1', offvalue='')
        ...
        # save button
        self.jt_btn_save_job = tk.Button(
            self.frame, text="Save Job", command=self.save_job_to_excel)
        self.jt_btn_save_job.configure(background='#ffffff')

        # Combobox
        self.job_name = tk.StringVar()
        self.jt_job_name_combo = ttk.Combobox(
            self.frame, width=35, textvariable=self.job_name, postcommand=self.get_latest_combobox_values)
        ...
        self.jt_btn_open_in_excel = tk.Button(
            self.frame, text="Open in Excel", command=self.open_in_excel)
    ...
    def get_combobox_values(self):
        ...
        # only return the 30 most recent jobs as the list gets too large
        return job_names[0:30]
    ...
    def choose_date(self):
        ...
        self.jt_date_btn['text'] = survey_date[4:6] + \
            "/" + survey_date[2:4] + "/" + survey_date[0:2]
    ...
    def save_job_to_excel(self):
        ...
        try:
            ...
            # Default value is create new job - required to set the combo box value after a refresh
            selected_job_index = -1

            if job_name == "<<Enter New Job>>":
                tk.messagebox.showerror(
                    "Survey Assist", "Please enter a job name")
                return
            ...
            workbook = load_workbook(
                self.job_tracker_filepath, read_only=False, keep_vba=True)
            ...
            if self.create_new_job:  # user is creating a new job
                ...
                for job in self.job_tracker.survey_job_list:
                    if job_name == job.job_name and job_date == job.survey_date:
                        tk.messagebox.showerror(
                            "Survey Assist", "This job name already exists with this date.  Please enter a unique job name/date")
                        return
                ...
                self.update_user(
                    actions_sheet["C11"], self.jt_user_entry.get())

                # apply 2-scale conditional formatting to check boxes
                actions_sheet.conditional_formatting.add(
                    "D11:G11", two_color_scale_rule)
                actions_sheet.conditional_formatting.add(
                    "H11", xml_two_color_scale_rule)
                ...
                for row in range(11, 11 + len(self.job_tracker.get_job_names())):
                    ...
                    # Change font
                    actions_sheet[percentage_complete_cell].font = Font(
                        color="4472C4", name="Webdings")
                ...
                for index, row in enumerate(actions_sheet.iter_rows(min_row=11, max_row=100, min_col=1, max_col=2), start=11):
                    ...
                    if job_name == row_job_name and job_date == row_job_date:
                        ...
                        # required to set the combo box value after a refresh
                        selected_job_index = int(excel_row_to_update) - 10
                        break

                # current_job_selected_line = self.jt_job_name_combo.current()
                # excel_row_to_update = str(10 + current_job_selected_line)

                self.update_job_date(
                    actions_sheet["B" + excel_row_to_update], self.jt_date_btn['text'])
                self.update_user(
                    actions_sheet["C" + excel_row_to_update], self.jt_user_entry.get())
                self.update_checkbox_values(actions_sheet, excel_row_to_update)
                actions_sheet["I" +
                              excel_row_to_update] = self.jt_notes_entry.get()
                # self.update_conditional_formatting(actions_sheet)
            ...
            logger.exception(
                'Job Tracker excel spreadsheet not found\n\n' + str(ex))

            tk.messagebox.showerror(
                "ERROR", "Unable to find the Job Tracker Spreadsheet at the following location:\n\n" + self.job_tracker_filepath)

        except PermissionError as ex:

            logger.exception(
                'Job Tracker excel spreadsheet currently in use\n\n' + str(ex))
            ...
            # Most likely an incorrect file was chosen
            logger.exception(
                'Error has occurred in JobTracker save_job_to_excel().\n\n' + str(ex))

            tk.messagebox.showerror(
                "ERROR", 'An unexpected error has occurred reading the excel Job Tracker.  Please contact the developer')

            gui_app.menu_bar.job_tracker_sub_menu.entryconfig(
                "Track/Create a Job", state="disabled")
            gui_app.job_tracker_bar.hide_job_tracker_bar()
    ...
    def update_checkbox_values(self, actions_sheet, row):
        ...
        if self.xml_checkbox_var.get() == '1':
            self.update_checkbox(
                actions_sheet["H" + row], 1, Font(color='FFFF00'))
        else:
            self.update_checkbox(
                actions_sheet["H" + row], "", Font(color='FFFF00'))
    ...
    def backup_job_tracker(self):
        ...
        try:
            ...
            job_tracker_backup_dir = os.path.join(
                survey_config.root_job_directory, survey_config.current_year, Job_tracker_backup_folder_name)

            if os.path.exists(job_tracker_backup_dir) == False:
                os.mkdir(job_tracker_backup_dir)
            date_string = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
            save_name = os.path.join(job_tracker_backup_dir, os.path.basename(
                self.job_tracker_filepath).split('.')[0] + '_' + date_string + '.xlsm')
            print(save_name)
            shutil.copyfile(self.job_tracker_filepath, save_name)
        except Exception as ex:

            logger.exception(
                'Error has occurred in backup_job_tracker\n\n' + str(ex))
...
class ListBoxFrame(tk.Frame):
    ...
    def __init__(self, master):
        ...
        # Add scrollbar
        vsb = ttk.Scrollbar(self.list_box_view, orient='vertical',
                            command=self.list_box_view.yview)
    ...
    def populate(self, formatted_lines, highlight_lines=[]):
        ...
        for formatted_line in formatted_lines:
            ...
            self.list_box_view.insert(
                "", "end", values=complete_line, tags=(tag,))
        ...
        self.list_box_view.tag_configure(
            self.orientation_tag, background='#D1FAC5')
        # self.list_box_view.tag_configure(self.highlight_tag, background='#ffff00')
        self.list_box_view.tag_configure(
            self.highlight_tag, background='#FFFF00')
    ...
    def delete_selected_rows(self, event):
        ...
        for selected_item in selected_items:
            line_numbers_to_delete.append(
                self.list_box_view.item(selected_item)['values'][0])
...
class CreateDatedDirectoryWindow:
    ...
    def __init__(self, master, selected_directory):
        ...
        self.ok_btn = tk.Button(
            container, text='OK', command=lambda: self.create_directory(todays_date))
        self.change_date_btn = tk.Button(
            container, text='Change date', command=self.change_date)

        self.question_lbl.grid(row=1, column=1, columnspan=2,
                               sticky='nesw', padx=30, pady=(20, 5))
        self.ok_btn.grid(row=2, column=1, sticky='nesw',
                         padx=(30, 5), pady=(0, 10))
        self.change_date_btn.grid(
            row=2, column=2, sticky='nesw', padx=(5, 30), pady=(0, 10))
    ...
    def create_directory(self, active_date):
        ...
        if os.path.exists(new_directory_path) == False:
            ...
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'OTHER'))
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'GPS'))
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'OUTPUT'))
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'TS'))
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'TS', 'TS60'))
            os.makedirs(os.path.join(os.path.join(
                self.selected_directory, active_date), 'TS', 'MS60'))
            os.makedirs(os.path.join(os.path.join(
            ...
            tk.messagebox.showinfo(
                "Create directory", "Dated directory created in:\n\n" + create_dated_folder)
            ...
            self.dialog_window.destroy()
            tk.messagebox.showwarning(
                "DATED FOLDER EXISTS", "A dated folder for this date already exists")
...
class ImportRailMonitoringFileWindow:
    ...
    def __init__(self, master):
        ...
        self.sorting_lbl = tk.Label(
            self.dialog_window, text="Select the total station used:")

        self.ts_id = tk.StringVar()
        self.ts_id.set("TS60")
        self.ts_used = ""
        self.radio_no_sort = tk.Radiobutton(
            self.dialog_window, text="TS60", value="TS60", var=self.ts_id)
        self.radio_sort_auto = tk.Radiobutton(
        ...
        self.area_column_entry.grid(
            row=1, column=1, sticky='w', columnspan=1, padx=50, pady=2)
        self.sorting_lbl.grid(row=2, column=1, sticky='w',
                              columnspan=3, padx=50, pady=(20, 2))
        self.radio_no_sort.grid(
            row=3, column=1, sticky='w', columnspan=3, padx=70, pady=2)
...
class PointNameWindow:
    ...
    def __init__(self, master):
        ...
        self.lbl = tk.Label(self.dialog_window,
                            text="Enter a new name for this point")
        self.new_point_name_entry = tk.Entry(self.dialog_window)
        self.btn1 = tk.Button(self.dialog_window,
                              text="UPDATE", command=self.change_point_name)
    ...
    def change_point_name(self):
        try:
            ...
            if len(new_point_name) < 16:
                ...
                tk.messagebox.showinfo(
                    "INPUT ERROR", "Point names must be less than 15 characters in length.")
        except Exception as ex:
            print("Problem opening up the GSI file\n\n" + str(ex))
            logger.exception(
                "An unexpected error has occurred\n\nchange_point_name()\n\n" + str(ex))
...
class PrismConstantUpdate:
    ...
    def __init__(self, master, line_numbers_to_amend=None):
        ...
        self.pc_type_label = tk.Label(
            self.dialog_window, text="Please select the prism constant: ")
        self.pc_batch_label = tk.Label(
            self.dialog_window, text="Please select the prism constant batch file to process: ")
        self.prism_constant_selected = ""
        self.pc_batch_file_selected = ""
        self.pc_column = tk.StringVar()
        self.pc_column_entry = ttk.Combobox(
            self.dialog_window, width=32, textvariable=self.pc_column, state='readonly')

        self.pc_column_entry['values'] = sorted(
            list(gsi.PC_DICT_REAL_VALUES.keys()))

    def build_fix_single_window(self):

        self.dialog_window.geometry(
            MainWindow.position_popup(self.master, 260, 120))
        self.pc_type_label.grid(row=0, column=1, columnspan=2, padx=25, pady=5)
        self.pc_column_entry.grid(
            row=1, column=1, columnspan=2, padx=25, pady=5)
    ...
    def build_batch_file_window(self):
        ...
        self.dialog_window.geometry(
            MainWindow.position_popup(self.master, 340, 130))
        self.pc_batch_label.grid(
            row=0, column=1, columnspan=2, padx=25, pady=5)
        self.pc_column_entry.grid(
            row=1, column=1, columnspan=2, padx=25, pady=5)
        self.pc_column_entry['values'] = pc_batch_file_list

        run_pc_batch_file_btn = tk.Button(
            self.dialog_window, text="Update", width=10, command=self.run_pc_batch_file)
    ...
    def ok(self):
        ...
        if not self.prism_constant_selected:
            tk.messagebox.showinfo(
                "Updating Prism Constant", "Please select a prism type if you want to update")
        ...
        self.point_name = gsi.get_formatted_line(
            self.line_numbers_to_amend[0])['Point_ID']

        # ask user if he wishes  to update all shots to this mark
        update_all_shots_to_mark = tk.messagebox.askquestion(
            'Update Prism Constants', 'Would you like to update all shots to this mark?')

        if update_all_shots_to_mark == 'yes':
            self.line_numbers_to_amend = gsi.get_point_name_line_numbers(
                self.point_name)

        # update each line to amend with coordinates
        for line_number in self.line_numbers_to_amend:
            corrections = self.get_prism_constant_corrections(
                line_number, self.prism_constant_selected)
            if corrections.get('error', ""):
                # something unexpected went wrong
                tk.messagebox.showinfo(
                    "Updating Prism Constant", "An unexpected has occurred during update.")
                return
    ...
    def run_pc_batch_file(self):
        ...
        if not self.pc_batch_file_selected:
            tk.messagebox.showinfo(
                "Updating Prism Constant", "Please select a prism constant batch file to process")
            self.dialog_window.lift()
        ...
        pc_batch_file_path = os.path.join(
            self.config_files_path, self.pc_batch_file_selected)
        ...
        for line_number, formatted_line in enumerate(gsi.formatted_lines, start=1):
            ...
            try:
                ...
                corrections = self.get_prism_constant_corrections(
                    line_number, new_prism_constant)
                if corrections.get('error', ""):
                    # something unexpected went wrong
                    tk.messagebox.showinfo(
                        "Updating Prism Constant", "An unexpected has occurred during update.")
    ...
    def get_prism_constant_corrections(self, line_number, prism_constant_selected):
        ...
        try:
            ...
            # PC is the same - notify user
            if old_pc == gsi.PC_DICT_GSI_VALUES[prism_constant_selected]:
            ...
            stn_line_number, stn_formatted_line = gsi.get_station_from_line_number(
                line_number)
            ...
            cost_thea_north = (old_northing - stn_northing) / \
                old_slant_distance
            ...
            new_horizontal_distance = math.sqrt(
                (new_east - stn_easting) ** 2 + (new_north - stn_northing) ** 2)
            new_height_difference = abs(stn_height - new_height)

            new_slant_distance = str(
                decimalize_value(new_slant_distance, precision))
            ...
            new_horizontal_distance = str(
                decimalize_value(new_horizontal_distance, precision))
            new_height_difference = str(
                decimalize_value(new_height_difference, precision))
...
class ChangeHeightWindow:
    ...
    def get_entered_height(entry_widget):  # e.g. self.new_target_height_entry
        ...
        try:
            ...
            tk.messagebox.showerror(
                "INPUT ERROR", "Please enter a valid number to 3 decimal places")
...
class TargetHeightWindow(ChangeHeightWindow):
    ...
    def __init__(self, master):
        ...
        self.lbl = tk.Label(self.dialog_window,
                            text="Enter new target height for this shot:  ")
        self.new_target_height_entry = tk.Entry(self.dialog_window)
        self.btn1 = tk.Button(self.dialog_window,
                              text="UPDATE", command=self.fix_target_height)

        self.lbl.grid(row=0, column=1, padx=(20, 2), pady=20)
        self.new_target_height_entry.grid(
            row=0, column=2, padx=(2, 2), pady=20)
        self.btn1.grid(row=0, column=3, padx=(10, 20), pady=20)
    ...
    def fix_target_height(self):
        ...
        try:
            ...
            new_target_height = self.get_entered_height(
                self.new_target_height_entry)
            self.dialog_window.destroy()
            ...
            if new_target_height != 'ERROR':
                ...
                for line_number in line_numbers_to_ammend:
                    ...
                    corrections = self.get_target_height_corrections(
                        line_number, new_target_height)
                    ...
                    for stn_gsi_line_number, stn_point_id in station_setup_dic.items():
                        ...
                        if point_id == stn_point_id and point_id not in stn_setup_elevation_updated:
                            new_elevation = str(decimalize_value(
                                corrections['83'], self.precision))
                            gsi.update_station_elevation(
                                stn_formatted_line_number, new_elevation)
                ...
                GUIApplication.refresh()
                tk.messagebox.showinfo(
                    "Survey Assist", "Target Height Updated")
            else:
    ...
    def get_target_height_corrections(self, line_number, new_target_height):
        ...
        new_height_difference = str(
            decimalize_value(new_height_difference, '3dp'))
...
class StationHeightWindow(ChangeHeightWindow):
    ...
    def __init__(self, master):
        ...
        self.lbl = tk.Label(self.dialog_window,
                            text="Enter new station height for this setup:  ")
        self.new_station_height_entry = tk.Entry(self.dialog_window)
        self.btn1 = tk.Button(self.dialog_window, text="UPDATE",
                              command=self.change_station_height)

        self.lbl.grid(row=0, column=1, padx=(20, 2), pady=20)
        self.new_station_height_entry.grid(
            row=0, column=2, padx=(2, 2), pady=20)
        self.btn1.grid(row=0, column=3, padx=(10, 20), pady=20)
    ...
    def change_station_height(self):
        ...
        try:
            ...
            if new_station_height != 'ERROR':
                ...
                stn_line_number = gui_app.list_box.list_box_view.item(selected_line)[
                    'values'][0]

                station_formatted_line = gsi.get_formatted_line(
                    stn_line_number)

                station_setup_dic = gsi.get_list_of_station_setups(
                    gsi.formatted_lines)
                ...
                new_station_height = str(
                    decimalize_value(new_station_height, 3))

                # get all shots including station
                station_shots_dict = gsi.get_all_shots_from_a_station_including_setup(
                    station_formatted_line['Point_ID'], stn_line_number - 1)
                ...
                for gsi_line_number, formatted_line in station_shots_dict.items():
                    ...
                    # should be a station but double check
                    if gsi.is_station_setup(formatted_line):
                        gsi.update_station_height(
                            formatted_line_number, str(new_station_height))
                    elif gsi.is_orientation_shot(formatted_line):
                        continue
                        ...
                        old_point_elevation = float(
                            formatted_line['Elevation'])
                        new_point_elevation = old_point_elevation + \
                            float(stn_height_diff)
                        new_point_elevation = str(decimalize_value(
                            new_point_elevation, self.precision))
                        gsi.update_elevation(
                            formatted_line_number, new_point_elevation)

                        old_height_diff = float(formatted_line['Height_Diff'])
                        height_diff = old_height_diff + float(stn_height_diff)
                        height_diff = str(decimalize_value(
                            height_diff, self.precision))
                        gsi.update_height_diff(
                            formatted_line_number, height_diff)
                ...
                for formatted_line_number, coordinate_list in self.new_stn_coordinates.items():
                    ...
                    formatted_line = gsi.get_formatted_line(
                        formatted_line_number)

                    # should be a station but double check
                    if gsi.is_station_setup(formatted_line):
                        new_elevation = str(decimalize_value(
                            coordinate_list[2], self.precision))
                        gsi.update_station_elevation(
                            formatted_line_number, new_elevation)
                    else:
                        raise Exception(
                            "Unexpected error changing station height")
                ...
                GUIApplication.refresh()
                tk.messagebox.showinfo(
                    "Survey Assist", "Station Height Updated")
            else:
    ...
    def update_all_shot_elevations_from_station(self, line_number, station, elevation_diff):
        ...
        # get all shots including station
        station_shots_dict = gsi.get_all_shots_from_a_station_including_setup(
            station, line_number - 1)

        for gsi_line_number, formatted_line in station_shots_dict.items():
            formatted_line_number = gsi_line_number + 1

            # we dont update the elevation for a station setup
            if gsi.is_station_setup(formatted_line):
                continue
                ...
                change_coordinates_dict[formatted_line_number] = [
                    float(point_easting), float(point_northing), float(new_point_elevation)]
...
class CompnetUpdateFixedFileWindow:
    ...
    def __init__(self, master):
        ...
        self.fixed_btn = tk.Button(
            container, text='(1) Choose Fixed File: ', command=self.get_fixed_file_path)
        self.coord_btn = tk.Button(container, text='(2) Choose Coordinate File: ',
                                   command=self.get_coordinate_file_path)
        self.update_btn = tk.Button(
            container, text='(3) UPDATE FIXED FILE ', command=self.update_fixed_file)

        self.fixed_btn.grid(row=2, column=1, sticky='nesw',
                            padx=60, pady=(20, 3))
        self.coord_btn.grid(row=3, column=1, sticky='nesw', padx=60, pady=3)
        self.update_btn.grid(
            row=4, column=1, sticky='nesw', padx=60, pady=(3, 20))

        container.pack(fill="both", expand=True)

        self.dialog_window.geometry(
            MainWindow.position_popup(master, 240, 180))
    ...
    def update_fixed_file(self):
        ...
        try:
            ...
            coordinate_file = CoordinateFile.getCordinateFile(
                self.coordinate_file_path, coordinate_type)
            stations_updated = fixed_file.update(coordinate_file)
            stations_not_updated = set(
                fixed_file.get_stations()).difference(stations_updated)
            ...
            msg_body = str(len(stations_updated)) + \
                ' station coordinates have been updated.\n\n'
    ...
    def get_coordinate_file_path(self):
        ...
        if self.coordinate_file_path != "":
            self.coord_btn.config(
                text=os.path.basename(self.coordinate_file_path))
        self.dialog_window.lift()  # bring window to the front again
...
class CompnetWeightSTDFileWindow:
    ...
    def __init__(self, master):
        ...
        self.choose_btn = tk.Button(
            container, text="(1) Choose STD (weighted) File", command=self.get_STD_file_path)
        self.set_weighting_lbl = tk.Label(container, text="(2) Set Weighting:")
        self.entry_east = tk.Entry(
            container, width=5, textvariable=self.defualt_weight_sve)
        self.entry_east_lbl = tk.Label(container, text="Easting")
        self.entry_north = tk.Entry(
            container, width=5, textvariable=self.defualt_weight_svn)
        self.entry_north_lbl = tk.Label(container, text="Northing")
        self.entry_elevation = tk.Entry(
            container, width=5, textvariable=self.defualt_weight_svel)
        self.entry_elevation_lbl = tk.Label(container, text="Elevation")
        self.update_btn = tk.Button(
            container, text="(3) UPDATE", command=self.update_STD_file, anchor='w')

        self.choose_btn.grid(row=1, column=1, columnspan=3,
                             sticky='nesw', padx=40, pady=(20, 3))
        self.set_weighting_lbl.grid(
            row=2, column=1, columnspan=3, sticky='nsw', padx=40, pady=(10, 3))
        self.entry_east.grid(
            row=3, column=1, sticky='nesw', padx=(60, 2), pady=3)
        self.entry_east_lbl.grid(
            row=3, column=2, sticky='nesw', padx=(2, 40), pady=3)
        self.entry_north.grid(
            row=4, column=1, sticky='nesw', padx=(60, 2), pady=3)
        self.entry_north_lbl.grid(
            row=4, column=2, sticky='nesw', padx=(2, 40), pady=3)
        self.entry_elevation.grid(
            row=5, column=1, sticky='nesw', padx=(60, 2), pady=3)
        self.entry_elevation_lbl.grid(
            row=5, column=2, sticky='nesw', padx=(2, 40), pady=3)
        self.update_btn.grid(row=6, column=1, columnspan=3,
                             sticky='nesw', padx=40, pady=(10, 3))
    ...
    def get_STD_file_path(self):
        ...
        if self.std_file_path != "":
            ...
            gui_app.menu_bar.compnet_working_dir = os.path.dirname(
                self.std_file_path)
    ...
    def update_STD_file(self):
        try:
            if self.std_file_path != "":
                ...
                try:
                    updated_std_contents = std_file.update_weighting(
                        weight_dict)
                except InvalidOperation:
                    tkinter.messagebox.showinfo(
                        "Update STD File", "Please double check all weightings are a number")
                    self.dialog_window.lift()
                else:
                    # write out file
                    with open(self.std_file_path, "w") as std_file:
                        for line in updated_std_contents:
                            std_file.write(line)
                    tkinter.messagebox.showinfo(
                        "Update STD File", "STD file has been updated")
                    self.dialog_window.destroy()
            else:
                # user hasn't choosen a file
                tkinter.messagebox.showinfo(
                    "Update STD File", "Please choose an STD file")
                self.dialog_window.lift()
...
class CompnetCompareCRDFWindow:
    ...
    def __init__(self, master):
        ...
        self.compare_crd_files_lbl.grid(
            row=0, column=1, columnspan=2, padx=50, pady=2)
        self.tolE_lbl.grid(row=1, column=1, sticky='nesw',
                           padx=(25, 5), pady=3)
        self.entry_tolE.grid(
            row=1, column=2, sticky='nesw', padx=(5, 25), pady=2)
        self.tolN_lbl.grid(row=2, column=1, sticky='nesw',
                           padx=(25, 5), pady=3)
        self.entry_tolN.grid(
            row=2, column=2, sticky='nesw', padx=(5, 25), pady=3)
        self.tolH_lbl.grid(row=3, column=1, sticky='nesw',
                           padx=(25, 5), pady=3)
        self.entry_tolH.grid(
            row=3, column=2, sticky='nesw', padx=(5, 25), pady=3)

        self.crd_file_1_btn.grid(
            row=5, column=1, columnspan=2, sticky='nesw', padx=25, pady=(25, 3))
        self.crd_file_2_btn.grid(
            row=6, column=1, columnspan=2, sticky='nesw', padx=25, pady=3)
        self.compare_crd_btn.grid(
            row=7, column=1, columnspan=2, sticky='nesw', padx=25, pady=(3, 25))
    ...
    def compare_crd_files_outliers(self):
        ...
        try:
            ...
            for point in common_points:
                cf1_E = float(
                    coordinate_file1.coordinate_dictionary[point]['Eastings'])
                cf1_N = float(
                    coordinate_file1.coordinate_dictionary[point]['Northings'])
                cf2_E = float(
                    coordinate_file2.coordinate_dictionary[point]['Eastings'])
                cf2_N = float(
                    coordinate_file2.coordinate_dictionary[point]['Northings'])

                diff_E = cf1_E - cf2_E
                diff_N = cf1_N - cf2_N
                if abs(diff_E) > tol_E:
                    self.outliers_dict[point +
                                       ' - Easting'] = '{0:.3f}'.format(round(diff_E, 3))
                if abs(diff_N) > tol_N:
                    self.outliers_dict[point +
                                       ' - Northing'] = '{0:.3f}'.format(round(diff_N, 3))

                try:
                    # check and see if elevation data exists
                    cf1_H = float(
                        coordinate_file1.coordinate_dictionary[point]['Elevation'])
                    cf2_H = float(
                        coordinate_file2.coordinate_dictionary[point]['Elevation'])
                    diff_H = cf1_H - cf2_H

                    if abs(diff_H) > tol_H:
                        self.outliers_dict[point +
                                           ' - Elevation'] = '{0:.3f}'.format(round(diff_H, 3))
    ...
    def get_crd_file_path(self, file_path_number):
        ...
        parent_dated_directory = Path(
            survey_config.todays_dated_directory).parent
        ...
        if file_path_number == 1:
            ...
            if self.crd_file_path_1 != "":
                self.crd_file_1_btn.config(
                    text=os.path.basename(self.crd_file_path_1))
            self.dialog_window.lift()  # bring window to the front again
            ...
            if self.crd_file_path_2 != "":
                self.crd_file_2_btn.config(
                    text=os.path.basename(self.crd_file_path_2))
            self.dialog_window.lift()  # bring window to the front again
        else:
            tk.messagebox.showerror(
                "Error", "No filepath no exists: " + str(file_path_number))
...
class CompnetCreateControlOnlyGSI:
    ...
    def strip_non_control_shots(self):
        ...
        try:
            ...
            tk.messagebox.showerror(
                "ERROR", 'No GSI FIle Selected.  Please open a GSI file first')
...
class CombineGSIFilesWindow:
    ...
    def __init__(self, master):
        ...
        self.sorting_lbl = tk.Label(
            self.dialog_window, text="1)  SORTING OPTIONS:")
        ...
        self.sorted_file_btn = tk.Button(
            self.dialog_window, text='CHANGE SORTING CONFIG FILE', state="disabled", command=self.open_config_file)
        current_config_label_txt = os.path.basename(
            survey_config.sorted_station_config)
        self.current_config_label = tk.Label(
            self.dialog_window, text=current_config_label_txt, state="disabled")
        self.files_btn = tk.Button(self.dialog_window, text="2)  CHOOSE GSI'S AND COMBINE       ",
                                   command=self.select_and_combine_gsi_files)

        self.sorting_lbl.grid(row=0, column=1, sticky='w',
                              columnspan=3, padx=60, pady=(20, 2))
        self.radio_no_sort.grid(
            row=1, column=1, sticky='w', columnspan=3, padx=70, pady=2)
        self.radio_sort_auto.grid(
            row=2, column=1, sticky='w', columnspan=3, padx=70, pady=2)
        self.radio_sort_config.grid(
            row=3, column=1, sticky='w', columnspan=3, padx=70, pady=(2, 1))
        self.sorted_file_btn.grid(
            row=4, column=1, sticky='w', columnspan=3, padx=70, pady=(2, 2))
        self.current_config_label.grid(
            row=5, column=1, sticky='w', columnspan=3, padx=70, pady=(1, 2))
        self.files_btn.grid(row=6, column=1, sticky='w',
                            columnspan=3, padx=60, pady=(20, 20))

        self.dialog_window.geometry(
            MainWindow.position_popup(master, 320, 280))
    ...
    def open_config_file(self):
        ...
        if self.sorted_station_list_filepath != "":
            survey_config.update(SurveyConfiguration.section_config_files,
                                 'sorted_station_config', self.sorted_station_list_filepath)
            survey_config.sorted_station_config = self.sorted_station_list_filepath
            self.current_config_label.config(
                text=os.path.basename(self.sorted_station_list_filepath))
    ...
    def select_and_combine_gsi_files(self):
        ...
        try:
            ...
            if gsi_filenames:
                ...
                self.write_out_combined_gsi(
                    self.gsi_contents, self.combined_gsi_file_path)
                ...
                if radio_button_selection == "1":
                    ...
                    self.write_out_combined_gsi(
                        sorted_filecontents, self.combined_gsi_file_path)
                    ...
                    tk.messagebox.showerror(
                        "Error", "no radio button option choosed")
            ...
            tk.messagebox.showerror(
                "Error", "Error combining files.\n\n" + str(ex))

        else:

            if gsi_filenames:
                tk.messagebox.showinfo(
                    "Success", "The gsi files have been combined:\n\n" + self.combined_gsi_file_path)
    ...
    def sort_alphabetically(self):
        ...
        stations_names_dict = unsorted_combined_gsi.get_list_of_station_setups(
            unsorted_combined_gsi.formatted_lines)
        station_set = unsorted_combined_gsi.get_set_of_station_setups()
        if len(stations_names_dict) != len(station_set):
            tk.messagebox.showwarning(
                "WARNING", 'Warning - Duplicate station names detected!')

        # need to sort this by station name
        stations_sorted_by_name = OrderedDict(
            sorted(stations_names_dict.items(), key=lambda x: x[1]))
        ...
        for line_number, station_name in stations_sorted_by_name.items():
            ...
            line_numbers = unsorted_combined_gsi.get_all_shots_from_a_station_including_setup(
                station_name, line_number)
    ...
    def sort_by_config(self):
        ...
        if len(stations_names_dict) != len(station_set):
            tk.messagebox.showwarning(
                "WARNING", 'Warning - Duplicate station names detected in the combined gsi!')

        # need to sort this by station name
        stations_sorted_by_name_dict = OrderedDict(
            sorted(stations_names_dict.items(), key=lambda x: x[1]))
        ...
        for config_station in config_station_list:
            ...
            if config_station in stations_sorted_by_name:
                ...
                line_key_stations_sorted_by_name_dict = dict(
                    map(reversed, stations_sorted_by_name_dict.items()))
...
class JobDiaryWindow:
    ...
    def __init__(self, parent):
        ...
        self.type_path = os.path.join(
            survey_config.root_job_directory, survey_config.current_year)
        self.job_path = os.path.join(survey_config.root_job_directory,
                                     survey_config.current_year, survey_config.default_survey_type)

        self.types = sorted([f for f in os.listdir(
            self.type_path) if os.path.isdir(os.path.join(self.type_path, f))])
        self.jobs = sorted([f for f in os.listdir(
            self.job_path) if os.path.isdir(os.path.join(self.job_path, f))])
        ...
        self.menu_bar.add_command(
            label='Edit Record', command=self.edit_record)
        self.menu_bar.add_command(
            label='Save Record', command=self.save_record)
        self.menu_bar.add_command(
            label='Delete Record', command=self.delete_record)
        ...
        self.ID_label = tk.Label(
            self.top_frame, width=10, textvariable=self.ID)
        self.ID_label.pack(side=tk.LEFT)
        self.ID.set('--')

        self.job_type_box = ttk.Combobox(
            self.top_frame, textvariable=self.job_type, width=25)
        self.job_type_box.pack(side=tk.LEFT)
        self.job_type_box.bind('<<ComboboxSelected>>', self.refresh_jobs)

        self.job_name_box = ttk.Combobox(
            self.top_frame, textvariable=self.job_name, width=25)
        self.job_name_box.pack(side=tk.LEFT)

        self.job_date_btn = ttk.Button(
            self.top_frame, textvariable=self.job_date, comman=self.open_calender)
        ...
        self.chk_RTK = tk.Checkbutton(
            self.top_frame, text='RTK', variable=self.RTK)
        self.chk_static = tk.Checkbutton(
            self.top_frame, text='STATIC', variable=self.static)

        self.chk_TS15 = tk.Checkbutton(
            self.top_frame, text='TS 15', variable=self.TS15)
        self.chk_TS60 = tk.Checkbutton(
            self.top_frame, text='TS 60', variable=self.TS60)
        self.chk_MS60 = tk.Checkbutton(
            self.top_frame, text='MS 60', variable=self.MS60)
        self.chk_TS16 = tk.Checkbutton(
            self.top_frame, text='TS 16', variable=self.TS16)

        self.chk_other = tk.Checkbutton(
            self.top_frame, text='OTHER', variable=self.other_instrument)
        self.chk_manual = tk.Checkbutton(
            self.top_frame, text='MANUAL', variable=self.manual)
        self.chk_photos = tk.Checkbutton(
            self.top_frame, text='PHOTOS', variable=self.photos)
        ...
        self.chk_TS16.pack(side=tk.LEFT)
        ...
        ent_lbl = tk.Label(
            self.EntryFrame, text='Job Description...', width=25)
        ent_lbl.pack(side=tk.LEFT)

        self.Entry = tk.Entry(
            self.EntryFrame, textvariable=self.job_description)
        ...
        self.diary_entries = ttk.Treeview(self.bottom_frame,
                                          columns=('ID', 'Date', 'Job Name',
                                                   'Description', 'Job Type'),
                                          displaycolumns=(
                                              'Date', 'Job Name', 'Description', 'Job Type'),
                                          selectmode='browse')
        ...
        self.yscrollbar = ttk.Scrollbar(
            self.bottom_frame, orient='vertical', command=self.diary_entries.yview)
        self.diary_entries.configure(yscrollcommand=self.yscrollbar.set)
        self.yscrollbar.pack(side=tk.LEFT, expand=True,
                             fill='both', anchor='w')
    ...
    def populate_diary(self):
        ...
        for item in self.diary_data:
            try:
                self.diary_entries.insert(
                    '', 'end', iid=item['RECID'], text=item['RECID'], values=list(item.values()))
            except:
                pass

    def refresh_jobs(self, event):
        path = os.path.join(survey_config.root_job_directory,
                            survey_config.current_year, self.job_type.get())
        self.jobs = sorted([f for f in os.listdir(
    ...
    def activate_record(self, event):
        selected_record = self.diary_entries.item(
            self.diary_entries.focus())['values']
        ...
        if selected_record[5] == 1:
            self.chk_static.select()
        if selected_record[6] == 1:
            self.chk_RTK.select()
        if selected_record[7] == 1:
            self.chk_TS15.select()
        if selected_record[8] == 1:
            self.chk_TS60.select()
        if selected_record[9] == 1:
            self.chk_MS60.select()
        if selected_record[10] == 1:
            self.chk_TS16.select()
        if selected_record[11] == 1:
            self.chk_other.select()
        if selected_record[12] == 1:
            self.chk_manual.select()
    ...
    def disable_checks(self):
        ...
        self.chk_TS16.configure(state=tk.DISABLED)
    ...
    def enable_checks(self):
        ...
        self.chk_TS16.configure(state=tk.NORMAL)
    ...
    def save_record(self):
        ...
        if self.new_or_edited == 'New':
            self.diary_data.append(OrderedDict((('RECID', int(self.ID.get())), ('Date', self.job_date.get()),
                                                ...
                                                ('TS16', self.TS16.get()), ('Other',
                                                                            self.other_instrument.get()),
...
class CalendarWindow:
    ...
    def selection(self, day, name):
        ...
        self.year_selected = self.year
        self.day_name = name
        self.date_selected = str(self.day_selected) + \
    ...
    def setup(self, y, m):
        ...
        header = tk.Label(self.parent, height=2, text='{}   {}'.format(
            calendar.month_abbr[m], str(y)))
        ...
        ok = tk.Button(self.parent, width=5, text='OK',
...
def main():
    ...
    logger.info('************************* STARTED APPLICATION - User: ' +
                gui_app.menu_bar.user_config.user_initials + ' *************************')
